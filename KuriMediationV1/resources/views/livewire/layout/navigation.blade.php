...
new class extends Component
{
    ...
    {
        $this->currentYear = Year::whereNotNull('current_year')
            ->where('user_id', Auth::id())
            ->select('current_year') // Specify only the columns you need
            ->first();
    }
...
<nav x-data="{ open: false }" class="">
        ...
        <div class="pt-2 px-2 inline-flex w-full justify-between items-center">            
            <div class="sm:flex">
                ...
                <div class="sm:flex items-center">
                    <a href="{{ route('dashboard') }}" wire:navigate>
                        <span class="sm:pr-7 text-2xl font-bold">KuriBudget</span>
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center border border-transparent rounded-3xl ">
                            <img class="rounded-3xl sm:ml-3" height="50px" width="50px" src="{{ asset(auth()->user()->profile_pic_path) }}" alt="ProfilePicture">
                        </button>
                    </x-slot>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="px-3">
                <div class="font-medium text-base text-black" x-data="{{ json_encode(['username' => auth()->user()->username]) }}" x-text="username" x-on:profile-updated.window="username = $event.detail.username"></div>
            </div>
